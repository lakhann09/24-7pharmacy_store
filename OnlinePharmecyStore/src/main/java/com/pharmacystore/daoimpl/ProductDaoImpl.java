...
public class ProductDaoImpl implements ProductDao {
	...
	@Override
	public int getQuantityOfProduct(int pid) {
		try (Connection con = DbConnection.getDatabaseConnection()) {
	...
	@Override
	public boolean updateQuantityAfterCancellation(int pid, int quantity) {
		...
		try (Connection con = DbConnection.getDatabaseConnection()) {
			...
			ResultSet rs = pst.executeQuery();
			rs.next();
			int prev_qty = rs.getInt("quantity");

			pst = con.prepareStatement("UPDATE " + "product SET quantity = ? WHERE " + "productid = ?");

			pst.setInt(1, prev_qty + quantity);
			pst.setInt(2, pid);
			int count = pst.executeUpdate();
			if (count > 0)
				return true;
